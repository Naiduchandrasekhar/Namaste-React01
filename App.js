...
const Header = () => {
  return(
      <div className="header">
        <div>
        <img className="res-logo" src="https://image.similarpng.com/very-thumbnail/2021/09/Good-food-logo-design-on-transparent-background-PNG.png" />
        </div>
        <div>
          <ul>
            <li>Home</li>
...
const RestaurantCard = (props) => {
  const {resData} = props
  const {data} = resData
  const {name, cloudinaryImageId, cuisines, costForTwo, deliveryTime, totalRatingsString} = data
  return(
    <div className="card">
      <img className="images" src= {`https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/${cloudinaryImageId}`} />
      <h4>{name}</h4> 
      <h4>{totalRatingsString}</h4>
      <h5>{cuisines.join(", ")}</h5>
      <h5>{costForTwo/100} Rupees</h5>
      <h5>{deliveryTime} Mins Delivery</h5>
    </div>
  )
}
